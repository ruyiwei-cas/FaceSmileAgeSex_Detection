...
char *CLOTHE_HIST_PATH="ClothesFeature";
...
/// Acknowledge		: None
///
/// Function name	: GetClothesFeature
...
void GetImageHist(IplImage *HSVImg, double *Hist)
{
	...
	int nHist_Dim = ATTRIBUTE_FEATURE_DIM;
...
//		cvSaveImage(sBodyImageFilename, BodyImage);
	...
	GetImageHist(HSVImg, DetectionResult->Attribute_Feature);
...
void SavePersonCothesHistogram(vector<ClothesHist> &PersonClothesHist)
{
	...
	sprintf(sFilename,"%s\\Index.dat", sFolderName);
	...
	FILE *fIndexFile = fopen(sFilename, "wb+");
	//fprintf(fIndexFile, "Size:%d\n", nSize);
	fwrite(&nSize, sizeof(int), 1, fIndexFile);
	...
	for(int i=0;i<nSize;i++)
	{
		...
		fwrite(sText, sizeof(char), 20, fIndexFile);
		sprintf(sFilename, "%s\\%s.dat", sFolderName, sText);
#endif
#ifdef __linux__
		char sDate[11];
...
#endif
		// save histogram file
		fFile = fopen(sFilename, "wb+");
		//for (int j = 0; j<ATTRIBUTE_FEATURE_DIM; j++)
		//	fprintf(fFile,"%f\n", PersonClothesHist[i].HSV_Hist[j]);
		fwrite(PersonClothesHist[i].HSV_Hist, sizeof(double), ATTRIBUTE_FEATURE_DIM, fFile);
		fclose(fFile);
	}	
	fclose(fIndexFile);
}
...
int LoadPersonCothesHistogram(int nPersonID, vector<ClothesHist> &PersonClothesHist)
{
	...
	sprintf(sFilename,"%s\\Index.dat", sFolderName);
	FILE *fIndexFile = fopen(sFilename, "rb");
	if(fIndexFile ==NULL)
		return 0;
	//int nReadNum = fscanf(fIndexFile, "Size:%d\n", &nSize);
	int nReadNum = fread(&nSize, sizeof(int), 1, fIndexFile);
	if(nSize ==0)
	{
		fclose(fIndexFile);
		return nSize;
	...
	for(int i=0;i<nSize;i++)
	{
		...
		nReadNum = fread(sDate, sizeof(char), 20, fIndexFile);
		
		ClothesHist AverageHist;
		AverageHist.FaceID = nPersonID;
		AverageHist.Pic_date = DateString_to_CTime(sDate);

		sprintf(sFilename,"%s\\%s.dat",sFolderName, sDate);

		fFile = fopen(sFilename, "rb");
		//for (int j = 0; j<ATTRIBUTE_FEATURE_DIM; j++)
		//	nReadNum = fscanf(fFile,"%lf\n", &(AverageHist.HSV_Hist[j]));
		nReadNum = fread(AverageHist.HSV_Hist, sizeof(double), ATTRIBUTE_FEATURE_DIM, fFile);
		fclose(fFile);
...
int ContextFeature_Collection(char *sFilename)
{
	...
	*firstdot1 = NULL;
	...
	int nFace_Num = LoadProfilingFile(sRecognitionResultFilename, FaceRecognitionResult, Face_Valid_Flag);
	for (int i = 0; i<nFace_Num; i++)
	{
		if ((FaceRecognitionResult[i].FaceView == 0) && (FaceRecognitionResult[i].FaceID >= 0) && (FaceRecognitionResult[i].Prob_FaceID >= 0.25))
		{
			ClothesHist hist;
			hist.FaceID = FaceRecognitionResult[i].FaceID;
			memcpy(hist.HSV_Hist, FaceRecognitionResult[i].Attribute_Feature, ATTRIBUTE_FEATURE_DIM*sizeof(double));
...
void PersonHist_Generation(vector<ClothesHist> &AllClothesHist, int nPersonID, vector<ClothesHist> &AverageClothesHistArray)
{
	...
	for(int i=0;i<nNumber;i++)
	{
		...
		nHist_Number = 0;
		for (int m = 0; m<ATTRIBUTE_FEATURE_DIM; m++)
...
/// Acknowledge		: None
///
/// Function name	: Init_ClothesFeature
...
/// Acknowledge		: None
///
/// Function name	: PredictID_With_Clothes_matching
/// Description	    : 
///
...
int PredictID_With_Clothes_matching(double *Hist, char *sDate, double* dScore, bool bSameDay)
{
	...
	for(int i=0;i<FACE_TEMPLATE_MAX_NUM;i++)
	{
		...
		for(int j=0;j<nSize;j++)
		{
			...
			{
				dCurrentScore = Histogram_matching(AllPersonClothesHist[i][j].HSV_Hist, Hist, ATTRIBUTE_FEATURE_DIM);
